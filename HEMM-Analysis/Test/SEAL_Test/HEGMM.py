...
class HEGMM:
    ...
    def oA(self, A):
        ...
        for i in range(self.m):
            oA += A[i*self.l+(i%self.l):(i+1)*self.l] + A[i*self.l:(i+1)*self.l-(self.l-i%self.l)]
            if self.n > self.l:
                oA += [0] * (self.n - self.l)
            elif self.n < self.l:
                raise Exception("No funciona en este caso")
        return oA
    #include <mutex>
    def tB(self, B):
        tB = []
        for i in range(self.l * self.n):
            tB += [B[(i+(i%self.n)*self.n)%(self.l*self.n)]]
        return tB
...
# ERROR: FALTA ARREGLAR EL CASO (m == n < l) - NO FUNCIONA
    ...
    def e_mxn_coment(self, crypt, k, vectDecrypt):
        k = k % self.l
        if k == 0 and self.l == self.n:
            return crypt, vectDecrypt
        else:
            msk1 = [1 if (i % self.n) < k + self.n - self.l else 0 for i in range(self.m * self.n)]
            ctmsk1 = self.multiply_vect_coment(crypt, msk1, vectDecrypt)
            vmsk1 = mult_list(vectDecrypt, msk1)
            print(f"mask_1:\n{msk1}\n{vectDecrypt}\n{vmsk1}")
            vmsk1 = rotar_list(vmsk1, k-self.l)
            print(vmsk1)
            msk2 = [1 if k <= (i % self.n) < self.l else 0 for i in range(self.m * self.n)]
            ctmsk1 = self.multiply_vect_coment(crypt, msk2, vectDecrypt)
            vmsk2 = mult_list(vectDecrypt, msk2)
            print(f"mask_1:\n{msk2}\n{vectDecrypt}\n{vmsk2}")
            ctmsk1 = self.evaluator()
            vmsk2 = rotar_list(vmsk2, k)
            print(vmsk2)
            ret = self.add_coment()
            


    def w_mxn_coment(self, crypt, k, vectDecrypt):
        if k == 0 and self.l == self.m:
            return crypt, vectDecrypt

    def multiplica_coment(self, a, b, vA, vB):
        ctA, vtA = self.e_mxn_coment(a, 0, vA)
        raise Exception("Testeando")
        ctB, vtB = self.w_mxn_coment(b, 0, vB)
        ctC = self.multiplica_coment(ctA, ctB, vtA, vtB)
        vtC = mult_list(vtA, vtB)
        for k in range(1, self.l):
            ctA, vtA = self.e_mxn_coment(a, k, vA)
            ctB, vtB = self.w_mxn_coment(b, k, vB)
            ctC = self.add_coment(ctC, self.multiplica_coment(ctA, ctB, vtA, vtB), vtC, mult_list(vtA, vtB))
            vtC = sum_list(vtC, mult_list(vtA, vtB))
        print(self.compara(ctC, vtC))
        return ctC
